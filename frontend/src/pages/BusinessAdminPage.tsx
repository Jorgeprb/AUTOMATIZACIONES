...
export function BusinessAdminPage() {
  ...
  const [provisioningForm, setProvisioningForm] = useState({ assigned_number: "", provider: "voipstudio", external_provider_id: "", sip_target: "sip:bot@sip.autogal.es:6060;transport=udp", webhook_url: "", notes: "", status: "paid_pending_provisioning" });
  ...
  const provisioningClinicName = editingProvisioning
    ? accounts.data
        ?.flatMap((account) => account.clinics)
        .find((clinic) => clinic.id === editingProvisioning.clinic_id)?.name
    : null;
  ...
  return <div className="space-y-7">
    ...
    <Dialog open={Boolean(editingProvisioning)} onOpenChange={(open) => !open && setEditingProvisioning(null)}><DialogContent><DialogHeader><DialogTitle>{provisioningForm.status === "active" ? `Añadir número a ${provisioningClinicName ?? "la clínica"}` : "Gestionar provisión"}</DialogTitle><p className="text-sm leading-6 text-[#6f7b90]">Al guardar como activo, el número quedará asignado a la clínica y se enviará automáticamente un correo al cliente.</p></DialogHeader><div className="space-y-3">{([['assigned_number','Número asignado'],['provider','Proveedor'],['external_provider_id','ID externo'],['sip_target','SIP target'],['webhook_url','Webhook'],['notes','Notas']] as const).map(([key,label]) => <div key={key}><Label>{label}</Label><Input value={provisioningForm[key]} onChange={(event) => setProvisioningForm({ ...provisioningForm, [key]: event.target.value })} /></div>)}<Label>Estado</Label><select className="w-full rounded-md border p-2" value={provisioningForm.status} onChange={(event) => setProvisioningForm({ ...provisioningForm, status: event.target.value })}><option value="paid_pending_provisioning">Pago confirmado, pendiente</option><option value="provisioned">Aprovisionado</option><option value="active">Activo</option><option value="failed">Fallido</option></select><Button className="w-full" onClick={() => saveProvisioning.mutate()} disabled={saveProvisioning.isPending || (provisioningForm.status === "active" && !provisioningForm.assigned_number.trim())}><CreditCard className="size-4" />{provisioningForm.status === "active" ? "Asignar número y notificar" : "Guardar"}</Button></div></DialogContent></Dialog>
