...
export function BusinessAdminPage() {
  ...
  const [provisioningForm, setProvisioningForm] = useState({ assigned_number: "", provider: "voip_studio", external_provider_id: "", sip_target: "sip:bot@sip.autogal.es:6060;transport=udp", webhook_url: "", notes: "", status: "paid_pending_provisioning" });
  ...
  const saveProvisioning = useMutation({
    ...
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["provisioning"] }); setEditingProvisioning(null); toast.success("Provisión actualizada"); },
  ...
  const startProvisioning = (row: ProvisioningOrder) => {
    setEditingProvisioning(row);
    setProvisioningForm({ assigned_number: row.assigned_number ?? "", provider: row.provider ?? "voip_studio", external_provider_id: row.external_provider_id ?? "", sip_target: row.sip_target ?? "sip:bot@sip.autogal.es:6060;transport=udp", webhook_url: row.webhook_url ?? "", notes: row.notes ?? "", status: row.status });
  };
  ...
  return <div className="space-y-7">
    ...
    <Dialog open={Boolean(editingProvisioning)} onOpenChange={(open) => !open && setEditingProvisioning(null)}><DialogContent><DialogHeader><DialogTitle>Gestionar provisión</DialogTitle></DialogHeader><div className="space-y-3">{([['assigned_number','Número asignado'],['provider','Proveedor'],['external_provider_id','ID externo'],['sip_target','SIP target'],['webhook_url','Webhook'],['notes','Notas']] as const).map(([key,label]) => <div key={key}><Label>{label}</Label><Input value={provisioningForm[key]} onChange={(event) => setProvisioningForm({ ...provisioningForm, [key]: event.target.value })} /></div>)}<Label>Estado</Label><select className="w-full rounded-md border p-2" value={provisioningForm.status} onChange={(event) => setProvisioningForm({ ...provisioningForm, status: event.target.value })}><option value="paid_pending_provisioning">Pago confirmado, pendiente</option><option value="provisioned">Aprovisionado</option><option value="active">Activo</option><option value="failed">Fallido</option></select><Button className="w-full" onClick={() => saveProvisioning.mutate()} disabled={saveProvisioning.isPending}><CreditCard className="size-4" />Guardar</Button></div></DialogContent></Dialog>
