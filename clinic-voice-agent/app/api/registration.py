...
from app.enterprise_schemas import EmailRequest, OnboardingClinicRequest, PasswordResetRequest, RegisterRequest, TokenRequest
...
from app.models import AdminSession, AdminRole, AdminUser, Clinic
...
@router.post("/register", response_model=AdminIdentity, status_code=status.HTTP_201_CREATED)
def register(
    ...
    existing = session.scalar(select(AdminUser).where(or_(AdminUser.email == email, AdminUser.username == email)))
    ...
    _set_session_cookies(response, raw_token=raw_token, csrf_token=csrf_token, settings=settings)
    return _identity_for_user(session, user)


@router.post("/verify-email", status_code=204)
def verify_email(payload: TokenRequest, session: Annotated[Session, Depends(get_db)]) -> None:
...
def resend_verification(
    ...
    if user is not None and user.email_verified_at is None:
        raw = create_action_token(session, user_id=user.id, kind="verify_email", ttl=timedelta(hours=settings.email_verification_ttl_hours))
        enqueue_outbox(session, kind="email.send", dedupe_key=f"verify-email:{user.id}:{hashlib.sha256(raw.encode()).hexdigest()}", payload={"to": email, "subject": "Verifica tu correo de Autogal", "text": f"Verifica tu correo aquí: {_email_link(settings, '/verify-email', raw)}"})
        session.commit()
...
def forgot_password(
    ...
    if user is not None and user.is_active:
        raw = create_action_token(session, user_id=user.id, kind="reset_password", ttl=timedelta(minutes=settings.password_reset_ttl_minutes))
        enqueue_outbox(session, kind="email.send", dedupe_key=f"password-reset:{user.id}:{hashlib.sha256(raw.encode()).hexdigest()}", payload={"to": email, "subject": "Restablece tu contraseña de Autogal", "text": f"Restablece tu contraseña aquí: {_email_link(settings, '/reset-password', raw)}"})
        session.commit()
    return {"message": "Si la cuenta existe, recibirás un correo."}


@router.post("/reset-password", status_code=204)
def reset_password(payload: PasswordResetRequest, session: Annotated[Session, Depends(get_db)]) -> None:
    token = consume_action_token(session, raw_token=payload.token, kind="reset_password")
    if token is None:
        raise HTTPException(status_code=400, detail="El enlace no es válido o ha caducado.")
    user = session.get(AdminUser, token.user_id)
    if user is None:
...
def onboarding_clinic(
    ...
    if account is None:
        account = create_billing_account_for_user(session, user=user, display_name=user.display_name or user.username, billing_email=user.email or user.username)
