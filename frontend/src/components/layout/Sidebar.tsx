...
const adminClinicItems = [
  ...commonClinicItems.slice(0, 5),
  { label: "Asistente", icon: Bot, suffix: "assistant" },
  { label: "Flujos", icon: Workflow, suffix: "flows" },
  ...commonClinicItems.slice(5),
];
...
export function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <div className="flex h-full flex-col bg-white">
    ...
    {!isClientPortal ? <div className="border-t border-[#edf0f4] p-3"><NavItem to="/settings" label="Mi cuenta" icon={Settings} onNavigate={onNavigate} /></div> : null}
  </div>;
}
