  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Provisión de números</CardTitle></CardHeader><CardContent className="space-y-3">{commercial?.provisioning.length ? commercial.provisioning.map((item) => <div key={item.id} className="rounded-xl border p-4"><div className="flex justify-between gap-3"><strong>{item.assigned_number || "Número pendiente"}</strong><span className="text-xs font-semibold uppercase text-[#6c778b]">{item.status}</span></div><p className="mt-1 text-sm text-[#6e798c]">Cantidad: {item.quantity}</p><p className="mt-1 text-xs text-[#8390a3]">{item.status === "paid_pending_provisioning" ? "Pago confirmado. Estará activo en menos de 24 horas y recibirás un email." : item.status === "active" ? `Activo desde ${item.activated_at ? new Date(item.activated_at).toLocaleDateString("es-ES") : "hoy"}` : "En proceso de configuración"}</p></div>) : <p className="text-sm text-[#788396]">Todavía no hay solicitudes de provisión.</p>}</CardContent></Card>
