...
def _account_read(session: Session, account: BillingAccount) -> BillingAccountRead:
...
def catalog(
    ...
    session.commit()
    return [
        CatalogItem(product=product, prices=prices)
        for product, prices in catalog_rows(session)
    ]
...
def create_checkout(
    ...
    rows = list(
        session.scalars(
            select(BillingPrice).where(
                BillingPrice.id.in_(price_ids), BillingPrice.is_active.is_(True)
            )
        )
    )
    ...
    for line in payload.lines:
        ...
        if product is None or not price.stripe_price_id:
            raise HTTPException(
                status_code=503, detail="Catalog price is not synchronized with Stripe."
            )
    ...
    order.total_one_time_minor = one_time
    order.total_recurring_minor = recurring
...
def order_status(
    ...
    order = session.scalar(
        select(PurchaseOrder).where(
            PurchaseOrder.id == order_id, PurchaseOrder.billing_account_id == account.id
        )
    )
    if order is None:
...
def commercial_summary(
    ...
    orders = list(
        session.scalars(
            ...
            .where(PurchaseOrder.billing_account_id == account.id)
    ...
    subs = list(
        session.scalars(
            ...
            .where(ClinicSubscription.billing_account_id == account.id)
    ...
    payments = list(
        session.scalars(
            ...
            .where(PaymentRecord.billing_account_id == account.id)
    ...
    provisioning = list(
        session.scalars(
            ...
            .where(PhoneProvisioningOrder.billing_account_id == account.id)
            .order_by(PhoneProvisioningOrder.created_at.desc())
        )
    )
    entitlements = list(
        session.scalars(
            select(ClinicEntitlement).where(
                ClinicEntitlement.billing_account_id == account.id
            )
        )
    )
    phones = list(
        session.scalars(
            ...
            .where(
                Clinic.billing_account_id == account.id, PhoneNumber.is_active.is_(True)
            )
        )
    )
...
def customer_portal(
    ...
    if not account.stripe_customer_id:
        raise HTTPException(status_code=409, detail="No Stripe customer exists yet.")
...
def cancel_subscription(
    ...
    row = session.scalar(
        select(ClinicSubscription).where(
            ClinicSubscription.id == subscription_id,
